...
export const EMAILJS_CONFIG = {
  ...
  // Company notification email templates (MUST be different from customer templates)
  // IMPORTANT: These templates must have "To Email" field set to {{to_email}} and will receive company email
  // Create separate templates in EmailJS dashboard for company notifications
  TEMPLATE_ID_DELIVERY_NOTIFICATION: process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_DELIVERY_NOTIFICATION || 'template_t5zk1w1',
...
export async function sendDeliveryAddressNotificationEmail(
  ...
  try {
    ...
    const templateParams = {
      ...
      to_email: "pinkdotfashionjewellery@gmail.com",
...
This address will be used for future orders from this customer.`,
    ...
    console.log('Sending delivery address notification to company:', {
      ...
      template_configured: !!EMAILJS_CONFIG.TEMPLATE_ID_DELIVERY_NOTIFICATION,
      warning: EMAILJS_CONFIG.TEMPLATE_ID_DELIVERY_NOTIFICATION === EMAILJS_CONFIG.TEMPLATE_ID_DELIVERY_ADDRESS 
        ? 'WARNING: Using same template as customer email - create separate template for company notifications!' 
        : 'Using separate notification template',
    });
